...
struct Tie
    : public streambuf { // MSVC requires split streambuf for cin and cout
...
#define make_version_string(major, minor, patch)                               \
  ...
  compiler += "(version ";
  compiler +=
      stringify(__INTEL_COMPILER) " update " stringify(__INTEL_COMPILER_UPDATE);
  compiler += ")";
#elif _MSC_VER
  compiler += "MSVC ";
  compiler += "(version ";
...
void aligned_large_pages_free(void *mem) {
  ...
  if (mem && !VirtualFree(mem, 0, MEM_RELEASE)) {
    ...
    std::cerr << "Failed to free large page memory. Error code: 0x" << std::hex
