...
constexpr unsigned MAX_INDEX =
    2 * 24 * 64 * 64; // stm * psq * wksq * bksq = 196608
...
KPKPosition::KPKPosition(unsigned idx) {
  ...
  psq =
      make_square(File((idx >> 13) & 0x3), Rank(RANK_7 - ((idx >> 15) & 0x7)));

  // Invalid if two pieces are on the same square or if a king can be captured
  if (distance(ksq[WHITE], ksq[BLACK]) <= 1 || ksq[WHITE] == psq ||
      ksq[BLACK] == psq ||
      (stm == WHITE && (pawn_attacks_bb(WHITE, psq) & ksq[BLACK])))
    result = INVALID;

  // Win if the pawn can be promoted without getting captured
  else if (stm == WHITE && rank_of(psq) == RANK_7 &&
           ksq[WHITE] != psq + NORTH &&
  ...
  // Draw if it is stalemate or the black king can capture the pawn
           ...
           (!(attacks_bb<KING>(ksq[BLACK]) &
            ...
            (attacks_bb<KING>(ksq[BLACK]) & ~attacks_bb<KING>(ksq[WHITE]) &
             psq)))
    result = DRAW;
...
Result KPKPosition::classify(const std::vector<KPKPosition> &db) {
  ...
  // classified as DRAW, the current position is classified as DRAW, otherwise
  ...
  // classified as WIN, the position is classified as WIN, otherwise the current
