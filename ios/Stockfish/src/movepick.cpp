...
MovePicker::MovePicker(const Position &p, Move ttm, Depth d,
                       const ButterflyHistory *mh, const LowPlyHistory *lp,
  ...
  stage =
      (pos.checkers() ? EVASION_TT : MAIN_TT) + !(ttm && pos.pseudo_legal(ttm));
}
...
MovePicker::MovePicker(const Position &p, Move ttm, Depth d,
  ...
  stage = (pos.checkers() ? EVASION_TT : QSEARCH_TT) +
          !(ttm &&
            (pos.checkers() || depth > DEPTH_QS_RECAPTURES ||
  ...
  for (auto &m : *this)
    if constexpr (Type == CAPTURES)
      m.value = int(PieceValue[MG][pos.piece_on(to_sq(m))]) * 6 +
                (*captureHistory)[pos.moved_piece(m)][to_sq(m)]
      ...
      m.value = (*mainHistory)[pos.side_to_move()][from_to(m)] +
                ...
                (ply < MAX_LPH ? std::min(4, depth / 3) *
                                     (*lowPlyHistory)[ply][from_to(m)]
                               : 0);
...
top:
  ...
  case REFUTATION:
    if (select<Next>([&]() {
          return *cur != MOVE_NONE && !pos.capture(*cur) &&
