...
ExtMove *generate_pawn_moves(const Position &pos, ExtMove *moveList,
  ...
  const Bitboard emptySquares =
      Type == QUIETS || Type == QUIET_CHECKS ? target : ~pos.pieces();
  ...
  if (Type != CAPTURES) {
    ...
    if (Type == QUIET_CHECKS) {
      ...
      // Discovered check promotion has been already generated amongst the
      // captures.
...
ExtMove *generate_all(const Position &pos, ExtMove *moveList) {
  ...
  constexpr bool Checks =
      Type == QUIET_CHECKS; // Reduce template instantiations
  ...
  if (!Checks || pos.blockers_for_king(~Us) & ksq) {
    Bitboard b =
        attacks_bb<KING>(ksq) & (Type == EVASIONS ? ~pos.pieces(Us) : target);
    if (Checks)
...
/// <EVASIONS>     Generates all pseudo-legal check evasions when the side to
/// move is in check <QUIET_CHECKS> Generates all pseudo-legal non-captures
/// giving check, except castling and promotions <NON_EVASIONS> Generates all
/// pseudo-legal captures and non-captures
///
