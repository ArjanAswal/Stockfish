...
template <> Value Endgame<KBNK>::operator()(const Position &pos) const {
  ...
  Value result = (VALUE_KNOWN_WIN + 3520) + push_close(strongKing, weakKing) +
                 420 * push_to_corner(opposite_colors(strongBishop, SQ_A1)
                                          ? flip_file(weakKing)
                                          : weakKing);
...
template <> Value Endgame<KRKP>::operator()(const Position &pos) const {
  ...
  Square queeningSquare =
      make_square(file_of(weakPawn), relative_rank(weakSide, RANK_8));
  Value result;
  ...
  // it's a win.
  else if (distance(weakKing, weakPawn) >=
               3 + (pos.side_to_move() == weakSide) &&
  ...
  // the position is drawish
           ...
           distance(strongKing, weakPawn) >
               2 + (pos.side_to_move() == strongSide))
    result = Value(80) - 8 * distance(strongKing, weakPawn);

  else
    result =
        Value(200) - 8 * (distance(strongKing, weakPawn + pawn_push(weakSide)) -
...
template <> Value Endgame<KRKN>::operator()(const Position &pos) const {
  ...
  Value result =
      Value(push_to_edge(weakKing) + push_away(weakKing, weakKnight));
  return strongSide == pos.side_to_move() ? result : -result;
}
...
/// the defending side in the search, this is usually sufficient to win KQ vs
/// KR.
template <> Value Endgame<KQKR>::operator()(const Position &pos) const {
  ...
  Value result = QueenValueEg - RookValueEg + push_to_edge(weakKing) +
...
template <> Value Endgame<KNNKP>::operator()(const Position &pos) const {
  ...
  Value result = PawnValueEg + 2 * push_to_edge(weakKing) -
...
/// will be used.
template <> ScaleFactor Endgame<KBPsK>::operator()(const Position &pos) const {
  ...
  if (!(strongPawns & ~FileABB) || !(strongPawns & ~FileHBB)) {
    Square queeningSquare = relative_square(
        strongSide, make_square(file_of(lsb(strongPawns)), RANK_8));
  ...
  if ((!(allPawns & ~FileBBB) || !(allPawns & ~FileGBB)) &&
      pos.non_pawn_material(weakSide) == 0 && pos.count<PAWN>(weakSide) >= 1) {
    ...
    if (relative_rank(strongSide, weakPawn) == RANK_7 &&
        ...
        (opposite_colors(strongBishop, weakPawn) ||
         !more_than_one(strongPawns))) {
...
template <> ScaleFactor Endgame<KQKRPs>::operator()(const Position &pos) const {
...
template <> ScaleFactor Endgame<KRPKR>::operator()(const Position &pos) const {
  ...
  if (pawnRank <= RANK_5 && distance(weakKing, queeningSquare) <= 1 &&
      strongKing <= SQ_H5 &&
  ...
  if (pawnRank == RANK_6 && distance(weakKing, queeningSquare) <= 1 &&
      ...
      (rank_of(weakRook) == RANK_1 ||
       (!tempo && distance<File>(weakRook, strongPawn) >= 3)))
    return SCALE_FACTOR_DRAW;

  if (pawnRank >= RANK_6 && weakKing == queeningSquare &&
      rank_of(weakRook) == RANK_1 &&
  ...
  if (strongPawn == SQ_A7 && strongRook == SQ_A8 &&
      (weakKing == SQ_H7 || weakKing == SQ_G7) && file_of(weakRook) == FILE_A &&
  ...
  if (pawnRank == RANK_7 && pawnFile != FILE_A &&
      file_of(strongRook) == pawnFile && strongRook != queeningSquare &&
      (distance(strongKing, queeningSquare) <
       distance(weakKing, queeningSquare) - 2 + tempo) &&
      (distance(strongKing, queeningSquare) <
       distance(weakKing, strongRook) + tempo))
    return ScaleFactor(SCALE_FACTOR_MAX -
                       2 * distance(strongKing, queeningSquare));

  // Similar to the above, but with the pawn further back
  if (pawnFile != FILE_A && file_of(strongRook) == pawnFile &&
...
template <> ScaleFactor Endgame<KRPKB>::operator()(const Position &pos) const {
...
ScaleFactor Endgame<KRPPKRP>::operator()(const Position &pos) const {
  ...
  if (pos.pawn_passed(strongSide, strongPawn1) ||
      pos.pawn_passed(strongSide, strongPawn2))
    return SCALE_FACTOR_NONE;

  Rank pawnRank = std::max(relative_rank(strongSide, strongPawn1),
                           relative_rank(strongSide, strongPawn2));
...
/// are on the same rook file and are blocked by the defending king, it's a
/// draw.
template <> ScaleFactor Endgame<KPsK>::operator()(const Position &pos) const {
...
template <> ScaleFactor Endgame<KBPPKB>::operator()(const Position &pos) const {
  ...
  if (relative_rank(strongSide, strongPawn1) >
      relative_rank(strongSide, strongPawn2)) {
  ...
  case 0:
    ...
    if (file_of(weakKing) == file_of(blockSq1) &&
        relative_rank(strongSide, weakKing) >=
            relative_rank(strongSide, blockSq1) &&
        opposite_colors(weakKing, strongBishop))
      return SCALE_FACTOR_DRAW;
    else
      return SCALE_FACTOR_NONE;
  ...
  case 1:
    ...
    if (weakKing == blockSq1 && opposite_colors(weakKing, strongBishop) &&
        (weakBishop == blockSq2 ||
         (attacks_bb<BISHOP>(blockSq2, pos.pieces()) &
...
template <> ScaleFactor Endgame<KBPKN>::operator()(const Position &pos) const {
  ...
  if (file_of(weakKing) == file_of(strongPawn) &&
      relative_rank(strongSide, strongPawn) <
          relative_rank(strongSide, weakKing) &&
      (opposite_colors(weakKing, strongBishop) ||
       relative_rank(strongSide, weakKing) <= RANK_6))
    return SCALE_FACTOR_DRAW;
...
/// KP vs K bitbase: if the weakest side has a draw without the pawn, it
...
template <> ScaleFactor Endgame<KPKP>::operator()(const Position &pos) const {
  ...
  return Bitbases::probe(strongKing, strongPawn, weakKing, us)
             ? SCALE_FACTOR_NONE
             : SCALE_FACTOR_DRAW;
}
