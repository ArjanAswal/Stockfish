...
/// evaluate() calculates a score for the static pawn structure of the given
/// position. We cannot use the location of pieces or king in this function, as
/// the evaluation of the pawn structure will be stored in a small cache for
/// speed reasons, and will be re-used even when the pieces have moved.
...
template <Color Us> Score evaluate(const Position &pos, Pawns::Entry *e) {
  ...
  e->blockedCount +=
      popcount(shift<Up>(ourPawns) & (theirPawns | doubleAttackThem));
  ...
  while (b) {
    ...
    backward =
        !(neighbours & forward_ranks_bb(Them, s + Up)) && (leverPush | blocked);
    ...
    if (support | phalanx) {
      ...
      score -=
          Backward + WeakUnopposed * !opposed * bool(~(FileABB | FileHBB) & s);
...
Score Entry::evaluate_shelter(const Position &pos, Square ksq) const {
  ...
  // King On File
  bonus -= KingOnFile[pos.is_on_semiopen_file(Us, ksq)]
                     [pos.is_on_semiopen_file(Them, ksq)];

  return bonus;
}

/// Entry::do_king_safety() calculates a bonus for king safety. It is called
/// only when king square changes, which is about 20% of total king_safety()
/// calls.

template <Color Us> Score Entry::do_king_safety(const Position &pos) {
  ...
  if (pos.can_castle(Us & KING_SIDE))
    shelter =
        std::max(shelter, evaluate_shelter<Us>(pos, relative_square(Us, SQ_G1)),
                 compare);

  if (pos.can_castle(Us & QUEEN_SIDE))
    shelter =
        std::max(shelter, evaluate_shelter<Us>(pos, relative_square(Us, SQ_C1)),
                 compare);
