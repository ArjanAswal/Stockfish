...
constexpr Score QuadraticOurs[][PIECE_TYPE_NB] = {
    ...
    {S(-210, -211), S(37, 14), S(147, 141), S(161, 105), S(-158, -174),
     S(-9, -31)} // Queen
};
...
// Endgame evaluation and scaling functions are accessed directly and not
// through the function maps because they correspond to more than one material
// hash key.
Endgame<KXK> EvaluateKXK[] = {Endgame<KXK>(WHITE), Endgame<KXK>(BLACK)};

Endgame<KBPsK> ScaleKBPsK[] = {Endgame<KBPsK>(WHITE), Endgame<KBPsK>(BLACK)};
Endgame<KQKRPs> ScaleKQKRPs[] = {Endgame<KQKRPs>(WHITE),
...
template <Color Us> Score imbalance(const int pieceCount[][PIECE_TYPE_NB]) {
...
Entry *probe(const Position &pos) {
  ...
  e->gamePhase = Phase(((npm - EndgameLimit) * PHASE_MIDGAME) /
  ...
  if (npm_w + npm_b == VALUE_ZERO &&
      pos.pieces(PAWN)) // Only pawns on the board
  {
