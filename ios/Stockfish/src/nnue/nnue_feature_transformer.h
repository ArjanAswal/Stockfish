...
public:
  ...
  std::int32_t transform(const Position &pos, OutputType *output,
                         int bucket) const {
      ...
      for (IndexType j = 0; j < NumChunks; ++j) {
        ...
        _mm512_store_si512(
            &out[j], _mm512_permutexvar_epi64(
                         Control, _mm512_max_epi8(
                                      _mm512_packs_epi16(sum0, sum1), Zero)));
      }
    }
    return psqt;
...
#else
        _mm_store_si128(
            &out[j], _mm_subs_epi8(_mm_adds_epi8(packedbytes, k0x80s), k0x80s));
#endif
      }
    }
    return psqt;
...
private:
  void update_accumulator(const Position &pos, const Color perspective) const {
    ...
    if (st->accumulator.computed[perspective]) {
      ...
      FeatureSet::append_changed_indices(ksq, next, perspective, removed[0],
                                         added[0]);
      for (StateInfo *st2 = pos.state(); st2 != next; st2 = st2->previous)
        FeatureSet::append_changed_indices(ksq, st2, perspective, removed[1],
                                           added[1]);
      ...
      // Now update the accumulators listed in states_to_update[], where the
      ...
      for (IndexType j = 0; j < HalfDimensions / TileHeight; ++j) {
        auto biasesTile =
            reinterpret_cast<const vec_t *>(&biases[j * TileHeight]);
        for (IndexType k = 0; k < NumRegs; ++k)
          acc[k] = biasesTile[k];
