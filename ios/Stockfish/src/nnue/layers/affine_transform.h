...
template <typename PreviousLayer, IndexType OutDims> class AffineTransform {
public:
  // Input/output type
  ...
  // Size of the forward propagation buffer used from the input layer to this
  // layer
  ...
  bool read_parameters(std::istream &stream) {
    if (!previousLayer.read_parameters(stream))
      return false;
...
#else
      weights[(i / 4) % (PaddedInputDimensions / 4) * OutputDimensions * 4 +
  ...
  bool write_parameters(std::ostream &stream) const {
    if (!previousLayer.write_parameters(stream))
      return false;
    for (std::size_t i = 0; i < OutputDimensions; ++i)
    ...
    std::unique_ptr<WeightType[]> unscrambledWeights =
        std::make_unique<WeightType[]>(OutputDimensions *
  ...
  const OutputType *propagate(const TransformedFeatureType *transformedFeatures,
                              char *buffer) const {
    const auto input =
        previousLayer.propagate(transformedFeatures, buffer + SelfBufferSize);
    ...
    [[maybe_unused]] auto m512_add_dpbusd_epi32 = [=](__m512i &acc, __m512i a,
                                                      __m512i b) {
#if defined(USE_VNNI)
    ...
    [[maybe_unused]] auto m512_add_dpbusd_epi32x4 =
        [=](__m512i &acc, __m512i a0, __m512i b0, __m512i a1, __m512i b1,
    ...
    [[maybe_unused]] auto m256_add_dpbusd_epi32 = [=](__m256i &acc, __m256i a,
                                                      __m256i b) {
#if defined(USE_VNNI)
    ...
    [[maybe_unused]] auto m256_add_dpbusd_epi32x4 =
        [=](__m256i &acc, __m256i a0, __m256i b0, __m256i a1, __m256i b1,
    ...
    [[maybe_unused]] auto m128_add_dpbusd_epi32 = [=](__m128i &acc, __m128i a,
                                                      __m128i b) {
    ...
    [[maybe_unused]] auto m128_add_dpbusd_epi32x4 =
        [=](__m128i &acc, __m128i a0, __m128i b0, __m128i a1, __m128i b1,
...
#define vec_set_32 _mm_set1_epi32
    ...
    if constexpr (OutputDimensions % OutputSimdWidth == 0) {
      ...
      for (int i = 0; i < (int)NumChunks - 3; i += 4) {
        ...
        const auto col0 = reinterpret_cast<const vec_t *>(
            &weights[(i + 0) * OutputDimensions * 4]);
        const auto col1 = reinterpret_cast<const vec_t *>(
            &weights[(i + 1) * OutputDimensions * 4]);
        const auto col2 = reinterpret_cast<const vec_t *>(
            &weights[(i + 2) * OutputDimensions * 4]);
        const auto col3 = reinterpret_cast<const vec_t *>(
            &weights[(i + 3) * OutputDimensions * 4]);
        for (int j = 0; j * OutputSimdWidth < OutputDimensions; ++j)
          vec_add_dpbusd_32x4(outptr[j], in0, col0[j], in1, col1[j], in2,
                              col2[j], in3, col3[j]);
      }
