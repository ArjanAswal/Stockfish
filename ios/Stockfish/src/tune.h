...
///
/// If you have a my_post_update() function to run after values have been
/// updated, and a my_range() function to set custom Option's min-max values,
/// then you just remove the 'const' qualifiers and write somewhere below in the
/// file:
///
...
///   UPDATE_ON_LAST();
///
/// And the values update, including post update function call, will be done
/// only once, after the engine receives the last UCI option, that is the one
/// defined and created as the last one, so the GUI should send the options in
/// the same order in which have been defined.
...
public:
  ...
  static void init() {
    for (auto &e : instance().list)
      e->init_option(); // Deferred, due to UCI::Options access
    read_options();
  }
...
// Some macro magic :-) we define a dummy int variable that compiler initializes
// calling Tune::add()
#define STRINGIFY(x) #x
#define UNIQUE2(x, y) x##y
#define UNIQUE(x, y) UNIQUE2(x, y) // Two indirection levels to expand __LINE__
#define TUNE(...)                                                              \
  int UNIQUE(p, __LINE__) = Tune::add(STRINGIFY((__VA_ARGS__)), __VA_ARGS__)
